...
const BeamLineMaterial = ({ color = '#60a5fa', opacity = 0.15, speed = 1, ...props }) => {
...
export default function ThreeScene({ isVisible = true, beamSpeed = 0.5, maxConnectionsPerPoint = 4, count = 1500 }: ThreeSceneProps) {
  ...
  // Create connections between points with improved logic
  const connections = useMemo(() => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const hash = `${Math.floor(x/connectionDistance)},${Math.floor(y/connectionDistance)},${Math.floor(z/connectionDistance)}`;
      ...
      for (let dx = -1; dx <= 1; dx++) {
        for (let dy = -1; dy <= 1; dy++) {
          for (let dz = -1; dz <= 1; dz++) {
            const hash = `${Math.floor(x/connectionDistance) + dx},${Math.floor(y/connectionDistance) + dy},${Math.floor(z/connectionDistance) + dz}`;
  ...
  // Animation frame with enhanced effects
  useFrame((state, delta) => {
    ...
    if (ref.current) {
      ...
      // Mouse interaction effect with smooth interpolation
  ...
  return (
    <group>
      ...
      {/* Render connections with improved visuals */}
      {connections.map(([start, end], index) => {
        ...
        return (
          <line key={index}>
            ...
            <BeamLineMaterial color={hovered ? "#60a5fa" : "#9bdcfa"} opacity={0.05} speed={isVisible ? beamSpeed : 0}  />
          </line>
        );
      })}
    </group>
